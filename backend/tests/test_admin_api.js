...
async function runTests() {
  ...
  // 3. Admin login via /api/auth/login
  let adminToken = '';
  await test('3. Administrator login via POST /api/auth/login yields valid JWT', async () => {
    const res = await fetch(`${BASE_URL}/api/auth/login`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: 'admin@bis.gov.in', password: 'Admin@1234' })
    });
